...
const verify = async (token) => {
  try {
    ...
    errorMessage.value = 'An error occurred, or the verification link has expired. Please request a new verification email.'
  } finally {
...
const handleResend = async () => {
  ...
  try {
    ...
    resendMessage.value = 'If an account exists for this email, a new verification link has been sent.'
  } catch (err) {
  ...
  <div class="min-h-screen grid place-items-center bg-gray-0">
    <div class="w-full max-w-xl rounded-2xl bg-white p-6 text-center">
      ...
      <div v-if="isVerifying" class="py-8">
        <div class="mx-auto mb-3 h-8 w-8 border-4 border-gray-200 border-t-black rounded-full animate-spin"></div>
        <p>Verifying your email…</p>
      </div>

      <div v-else>
        <div v-if="isSuccess" class="py-6">
          <p class="text-green-600 font-medium mb-6">Your account has been successfully activated.</p>
          <button class="px-4 py-2 bg-black text-white rounded-lg" @click="goHome">Continue</button>
        </div>
        ...
        <div v-else class="py-4">
          ...
          <div class="mt-4 text-left">
            ...
            <p v-if="resendMessage" class="mt-3 text-sm text-gray-600">{{ resendMessage }}</p>
          </div>
        </div>
      </div>
    </div>
